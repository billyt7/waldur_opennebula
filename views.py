...
from waldur_opennebula.apps import OpenNebulaConfig
...
class ServiceViewSet(structure_views.BaseServiceViewSet):
    queryset = models.OpenNebulaService.objects.all()
...
class LimitViewSet(RetrieveModelMixin, GenericViewSet):
    ...
    However, currently OpenNebula virtual machine limits are stored as options in service settings.
    Therefore in order to implement frontent-side validation of VM configuration in deployment form,
    we need to get limits of OpenNebula service settings for service consumer.
    ...
    queryset = structure_models.ServiceSettings.objects.filter(
        type=OpenNebulaConfig.service_name
    )
    lookup_field = 'uuid'
...
class VirtualMachineViewSet(structure_views.BaseResourceViewSet):
    ...
    def vm_tools_are_running(vm):
        if vm.tools_state != models.VirtualMachine.ToolsStates.RUNNING:
            raise rf_serializers.ValidationError('OpenNebula Tools are not running.')
