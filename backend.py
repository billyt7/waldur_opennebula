...
from waldur_opennebula.client import OpenNebulaClient
...
class OpenNebulaBackend(ServiceBackend):
    ...
    def client(self):
        """
        Construct OpenNebula REST API client using credentials specified in the service settings.
        """
        client = OpenNebulaClient(self.host, verify_ssl=False)
    ...
    def soap_client(self):
        """
        Construct OpenNebula SOAP API client using credentials specified in the service settings.
        """
    ...
    def pull_templates(self):
        """
        Pull OpenNebula templates for virtual machine provisioning from content library
        using OpenNebula REST API to the local database.
        """
        try:
    ...
    def import_virtual_machine(self, backend_id, save=True, service_project_link=None):
        ...
        :rtype: :class:`waldur_opennebula.models.VirtualMachine`
        """
        try:
    ...
    def _backend_vm_to_vm(self, backend_vm, tools_installed, tools_state, backend_id):
        ...
        :param tools_installed: whether OpenNebula tools installed or not
        :type tools_installed: bool
        :param tools_state: Status of OpenNebula Tools.
    ...
    def _get_template_nics(self, template):
        ...
        :type template: :class:`waldur_opennebula.models.Template`
        :rtype: list[str]
        """
    ...
    def is_virtual_machine_tools_running(self, vm):
        """
        Check OpenNebula tools status and update cache only if its running.
        If OpenNebula tools are not running, state is not updated.
        It is needed in order to skip extra database updates.
        Otherwise OpenNebula tools state in database would be updated
    ...
    def create_port(self, port):
        ...
        :type port: :class:`waldur_opennebula.models.Port`
        """
        try:
    ...
    def import_port(
        ...
        :rtype: :class:`waldur_opennebula.models.Disk`
        """
        try:
    ...
    def _backend_port_to_port(self, backend_port):
        ...
        :rtype: :class:`waldur_opennebula.models.Port`
        """
        return models.Port(
    ...
    def create_disk(self, disk):
        ...
        :type disk: :class:`waldur_opennebula.models.Disk`
        """
        spec = {
            'new_vmdk': {
    ...
    def delete_disk(self, disk, delete_vmdk=True):
        ...
        if delete_vmdk:
            ...
            try:
                ...
                logger.exception('Unable to delete OpenNebula disk. Disk ID: %s.', disk.id)
                raise OpenNebulaBackendError('Unknown error.')
            signals.vm_updated.send(self.__class__, vm=disk.vm)
    ...
    def extend_disk(self, disk):
        ...
        try:
            ...
            logger.exception('Unable to extend OpenNebula disk. Disk ID: %s.', disk.id)
            raise OpenNebulaBackendError('Unknown error.')
        signals.vm_updated.send(self.__class__, vm=disk.vm)
    ...
    def get_object(self, vim_type, vim_id):
        ...
        try:
            ...
            logger.exception(
                'Unable to get OpenNebula object. Type: %s, ID: %s.', vim_type, vim_id
            )
            raise OpenNebulaBackendError('Unknown error.')
        for item in items:
    ...
    def get_vm_tools_state(self, backend_id):
        """
        Get running status of OpenNebula Tools.
    ...
    def get_vm_tools_installed(self, backend_id):
        """
        Check if OpenNebula Tools are installed.
    ...
    def get_disk_datacenter(self, backend_disk):
        ...
        :return: OpenNebula datacenter where disk is located.
