...
class OpenNebulaService(structure_models.Service):
    projects = models.ManyToManyField(
        structure_models.Project, related_name='+', through='OpenNebulaServiceProjectLink'
    )

    class Meta:
        unique_together = ('customer', 'settings')
        verbose_name = _('OpenNebula provider')
        verbose_name_plural = _('OpenNebula providers')
...
class OpenNebulaServiceProjectLink(structure_models.ServiceProjectLink):

    service = models.ForeignKey(on_delete=models.CASCADE, to=OpenNebulaService)

    class Meta(structure_models.ServiceProjectLink.Meta):
        verbose_name = _('OpenNebula provider project link')
        verbose_name_plural = _('OpenNebula provider project links')

    @classmethod
    def get_url_name(cls):
        return 'opennebula-spl'
...
class VirtualMachine(
    ...
    tools_state = models.CharField(
        'Current running status of OpenNebula Tools running in the guest operating system.',
        max_length=50,
        blank=True,
    ...
    @classmethod
    def get_url_name(cls):
        return 'opennebula-virtual-machine'
